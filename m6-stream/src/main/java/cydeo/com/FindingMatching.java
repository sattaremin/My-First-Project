...
import cydeo.com.task.Dish;
import cydeo.com.task.DishData;
...
public class FindingMatching {
    ...
    public static void main(String[] args) {
        ...
        Object Dish;
        if(DishData.getAll().stream().anyMatch(cydeo.com.task.Dish::isVegetarian)){
        ...
        Optional<Dish> dish = DishData.getAll().stream().filter(cydeo.com.task.Dish::isVegetarian).findAny();
        System.out.println(dish.get());

        System.out.println("FIND FIRST");
        Optional<Dish> dish2 = DishData.getAll().stream().filter(cydeo.com.task.Dish::isVegetarian).findFirst();
...
//        Optional<String> findAny = list2.parallelStream().filter(s -> s.startsWith("J")).findAny();
        ...
        Optional<Dish> dMin = DishData.getAll().stream().min(Comparator.comparing(cydeo.com.task.Dish::getCalories));
        System.out.println(dMin.get());

        System.out.println("Max");
        Optional<Dish> dMax = DishData.getAll().stream().max(Comparator.comparing(cydeo.com.task.Dish::getCalories));
